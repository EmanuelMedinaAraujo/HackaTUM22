...
import { DetailsModal, setDetailsModal } from "../state/slices/quizSlice";
...
export default function MovieCardTop(props: Props) {
    ...
    const changeShowModal = () => {
        ...
        if (currentDetailsModal == DetailsModal.NONE) {
            dispatch(setDetailsModal(DetailsModal.TOP));
        } else {
            dispatch(setDetailsModal(DetailsModal.NONE));
        }
    }

    //Create css to ease the transiton in and out of the modal
    const style = currentDetailsModal != DetailsModal.BOTTOM ? { opacity: 1, transition: "opacity 2s ease-in-out" } : { opacity: 0, transition: "opacity 0.2s ease-in-out" };

    //@ts-ignore
    return (
        //Add blured background image
        <div className="relative">
            <div className={`z-40 card ${showModal ? "is-flipped" : ""} `} onClick={changeShowModal}>
                <div className="card-face card-face-front flex justify-center">
                        <div className="relative mt-5 h-fit" style={style}>
                            ...
                            <div className="bg-white rounded-full p-1 absolute -bottom-4 left-1/2 transform -translate-x-1/2">
                ...
                <div className={`card-face card-face-back`}>
                    <div className="bg-white p-10 m-10 rounded-box z-50" style={{ height: window.innerHeight / 2 - 50, display: "block", objectFit: "cover" }}>
